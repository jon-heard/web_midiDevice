...
class MouseController extends EventTarget {
	...
	constructor(canvas, size, axisPairCount) {
		super();
		this.canvas = canvas;
		...
		this.axisControlIds = Array(axisPairCount * 2).fill(0);
		...
		canvas.addEventListener('contextmenu', evt => { evt.preventDefault(); return false; });
	...
	renderMouseUp() {
		this.clearToColor('grey');
		this.setVecUniform(this.zoomBoxCenter, this.size / 2);
		this.renderZoomBox();
		this.renderValuePosition();
		this.renderValueLabels();
	}
	...
	renderStartTarget() {
		...
		this.context.fillRect(this.pointerDragPositionStart[0] - this.HALF_TARGET_SIZE, 0, this.TARGET_SIZE, this.size);
		this.context.fillRect(0, this.pointerDragPositionStart[1] - this.HALF_TARGET_SIZE, this.size, this.TARGET_SIZE);
	}
	...
	renderValuePosition() {
		...
		this.context.arc(this.zoomBoxCenter[0] + (values[0] - 0.5) * this.zoomSize, this.zoomBoxCenter[1] + (values[1] - 0.5) * this.zoomSize, this.POS_CIRCLE_SIZE, 0, this.PI2);
		this.context.fill();
	}
	...
	renderValueLabels() {
		...
		for (let axisId = 0; axisId < 2; axisId++) {
			const axisName = this.MIDI_CONTROL_NAMES[this.axisControlIds[axisId + this.pairId * 2]] || axisId;
